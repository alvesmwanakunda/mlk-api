...
async function uploadPlanProjetToFirebaseStorage(filename) {

  const path = `./public/${filename}`;
  const destination = `planprojet/${filename}`
...
async function deletePlanProjetFirebaseStorage(filename){

  try {
    await bucket.file(`planprojet/${filename}`).delete();
...
module.exports = {
  ...
  uploadPlanProjetToFirebaseStorage,
  deletePlanProjetFirebaseStorage
};
